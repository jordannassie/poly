...
export async function GET(request: NextRequest) {
  ...
  try {
    const { data, error } = await adminClient
      .from("coin_positions")
      .select("id, market_id, side, amount_coins, status, created_at, meta")
      .eq("user_id", userId)
      .order("created_at", { ascending: false });

    if (error) {
      console.error("Error fetching coin positions:", error);
      return NextResponse.json(
    ...
    console.error("Error in /api/coins/positions:", error);
    return NextResponse.json(
