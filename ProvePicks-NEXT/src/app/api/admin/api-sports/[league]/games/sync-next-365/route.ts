/**
 * POST /api/admin/api-sports/[league]/games/sync-next-365
 * 
...
import { syncGamesNextYear } from "@/lib/apiSports/gameSync";
...
export async function POST(
  ...
  try {
    const result = await syncGamesNextYear(
      adminClient,
    ...
    return NextResponse.json({
      ...
      message: `Synced ${result.totalGames} ${result.league} games for next 365 days (${result.inserted} new, ${result.updated} updated)`,
    });
  } catch (error) {
    const ms = Date.now() - startTime;
    return NextResponse.json({
