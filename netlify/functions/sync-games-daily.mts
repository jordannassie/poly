...
export default async (request: Request, context: Context) => {
  ...
  console.log("[cron] mode=daily status=started");
  
  if (!INTERNAL_CRON_SECRET) {
    console.error("[cron] mode=daily status=error reason=INTERNAL_CRON_SECRET_NOT_SET");
  ...
  try {
    ...
    console.log(`[cron] mode=daily status=${data.success ? 'ok' : 'error'} duration_ms=${duration_ms} games=${data.totalGames || 0} inserted=${data.totalInserted || 0} updated=${data.totalUpdated || 0}`);
    ...
    const duration_ms = Date.now() - startTime;
    console.error(`[cron] mode=daily status=error duration_ms=${duration_ms} error=${error instanceof Error ? error.message : 'Unknown'}`);
